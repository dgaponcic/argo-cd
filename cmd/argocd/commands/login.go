...
func oauth2LoginBrowserless(
	...
	deviceCodeRequest, err := http.NewRequest("POST", oidcSettings.DeviceURL, strings.NewReader(encodedData))
	...
	tokenRequest, err := http.NewRequest("POST", oidcSettings.TokenURL, strings.NewReader(encodedData))
	...
	if err != nil {
		fmt.Errorf("no token in response")
	}
